...
Song* addSong(Song* root, string name, int playCount) {
    if (!root) {
        return new Song(name, playCount);
    }
    if (playCount < root->playCount) {
        ...
        cout << "Song with play count " << playCount << " already exists." << endl;
    }
    return root;
}

Song* findSong(Song* root, int playCount) {
    if (!root || root->playCount == playCount) {
        return root;
    }
    if (playCount < root->playCount) {
        return findSong(root->lessPopular, playCount);
    } else {
        return findSong(root->morePopular, playCount);
    }
}

void searchSongWithCount(Song* root) {
    if (!root) {
        return;
    }
    searchSongWithCount(root->lessPopular);
    cout << root->name << " (" << root->playCount << " plays)" << endl;
    searchSongWithCount(root->morePopular);
}
...
int main() {
    ...
    cout << "Songs from least to most played:" << endl;
    searchSongWithCount(root);

    cout << "Songs removed in reverse popularity:" << endl;
    removeSongsReversePopularity(root);

    int searchCount = 30;
    Song* found = findSong(root, searchCount);
    if (found) {
        cout << "Song found: " << found->name << " (" << found->playCount << " plays)" << endl;
    } else {
        cout << "No song with play count " << searchCount << " found." << endl;
    }
    ...
    cout << "After removing play count " << deleteCount << ", songs from least to most played:" << endl;
    searchSongWithCount(root);
